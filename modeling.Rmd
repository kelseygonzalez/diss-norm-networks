...
pacman::p_load(tidyverse, here, lme4, equatiomatic, 
...
datasummary(formula = (`Percent White` = p_white)  +
  ...
  kable_styling(latex_options = c( "hold_position", "condensed", "striped", "repeat_header"))
...
google_h1 <- lmer(SAH ~ p_white + p_college + perc_65_over + 
...
google_h2 <- lmer(SAH ~ p_white + p_college + perc_65_over + 
                  income_med + unemployed_rate +
                  trump_votes_2016 + evangelical_county + 
                 fox_news + social_dist_trend + covid_conspiracy +
                  SAH_fips_mean + 
...
google_h3 <- lmer(SAH ~ p_white + p_college + perc_65_over + 
                  income_med + unemployed_rate +
                  trump_votes_2016 + evangelical_county + 
                 fox_news + social_dist_trend + covid_conspiracy +
                  SAH_fips_mean + 
...
vacc_h1 <- lmer(vacc_rate ~ p_white + p_college + perc_65_over + 
                  income_med + unemployed_rate +
                  trump_votes_2020 + evangelical_county + 
                  fox_news + covid_19_vaccine  + covid_conspiracy +
                  vacc_fips_mean + 
...
vacc_h2 <- lmer(vacc_rate ~ p_white + p_college + perc_65_over + 
                  income_med + unemployed_rate +
                  trump_votes_2020 + evangelical_county + 
                  fox_news + covid_19_vaccine  + covid_conspiracy +
                  vacc_fips_mean + 
...
vacc_h3 <- lmer(vacc_rate ~ p_white + p_college + perc_65_over + 
                  income_med + unemployed_rate +
                  trump_votes_2020 + evangelical_county + 
                  fox_news + covid_19_vaccine  + covid_conspiracy +
                  vacc_fips_mean + 
...
models using maximum likelihood with the lme4 package in R [@Bateslme4] for our
...
county. The lmer models are are optimized using Nelder–Mead, quasi-Newton and
conjugate-gradient algorithms for box-constrained optimization 
(`control = lmeControl(opt = "Nelder_Mead")`). This first model will address
...
modelsummary(list(google_h1, google_h2, google_h3),
             ...
             notes= c('* p < .05. ** p < .01. *** p < .001 (two-tailed test).',
                      ...
                      glue('N = {scales::label_comma()(nobs(google_h1))}, N of random Effects = {as.numeric(sapply(ranef(google_h1),nrow))}')),
...
`r broom.mixed::tidy(google_h1) %>% filter(term == 'case_rate') %>% pull(estimate) %>% round(4)` (_p_ = ), 
...
(_p_ < ). However, this coefficient is inadequate alone
...
(_p_ < ). The interaction between signal and signal discordance have a
...
modelsummary(list(vacc_h1, vacc_h2, vacc_h3),
             ...
             notes= c('* p < .05. ** p < .01. *** p < .001 (two-tailed test).',
                      'Model 1 includes a random intercept for FIPS,',
                      'Models 2-3 include a random effect for Movement Signal by FIPS',
                      glue('N = {scales::label_comma()(nobs(vacc_h1))}, N of random Effects = {as.numeric(sapply(ranef(vacc_h3),nrow))}')),
             # estimate = "{estimate}{stars}", 
