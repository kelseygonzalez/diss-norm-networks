...
theme_set(theme_diss()) # change theme_ben() with the wanted theme
...
variation in how individuals understand the threats of the disease
[@bailey_etal20], but it is also due to the current norms that are already in
place and that form in response to the policy. TODO: discuss norms, how it's hard to diverge from current norms, habitus 
<!-- TODO -->
...
- Write a sentence about [@bailey_etal20] 'We  show  that  social  network  exposure  to  COVID-19  cases  shapes  individuals’  beliefs  and  behaviors concerning the coronavirus.'
...
said risks (TODO CITE). However, while this model can be valid in specific
cases, more research has shown how the risks themselves do little to motivate
behavioral change (TODO CITE).
...
(@) _Hypothesis 2_ Being connected  Facebook friends living outside of the county will not lead to an increase in time spent in residence and decrease in vaccine uptake.
...
TODO Cite [@houghtonSocialContagionInteracting2021] in here somewhere
...
of U.S. counties to 210 media markets based on similar population clusters. Each
...
```
```{r vacc_models, dependson='load_vacc', cache = TRUE}
...
vacc_h1 <- lme(fixed = vacc_rate ~ p_white + p_college + perc_65_over + 
                  income_med + unemployed_rate +
                 evangelical_county + trump_votes_2020 + 
                 fox_news + covid_19_vaccine  + covid_conspiracy +
               case_rate + week_num, 
             random = ~ 1 | FIPS, 
             data = vacc,
...
(Vaccination Uptake), I fit series of three linear mixed effects regression
...
annealing (`control = lmeControl(opt = "optim")`). This model will address
Hypothesis 1. To test hypothesis 2, I estimate model 2 which builds on model 1
by first introducing the movement of vaccination signal. Because the signal
varies by week, I estimate this model with an interaction between week and
signal. Moreoever, because signal may have different effects across different
counties, I set a random effect for both signal. A visual representation of this
interaction can be seen in figures \@ref(fig:plot-google-h2) and
\@ref(fig:plot-vacc-h2). Model 3 then builds on model 2, introducing a fixed
effect for the interaction between signal and discordance*. Model 4 tests
hypothesis 4. Figures \@ref(fig:plot-google-h3) and \@ref(fig:plot-vacc-h3)
depict how signal and discordance interact.

* currently there is only an interaction because the vacc model won't converge because the interaction causes confounding. It may stay like ths, but leaving a TODO.
...
increased time spent in residence. In this first model, many controls have an
impact on the outcome. For instance, when controlling for everything else, the
counties' percent over 65 years old and Percent of GOP votes in 2016 decreased
the stay-at-home rate while higher median income and higher rates of
unemployment increased the stay-at-home rate. Searches for online norms seem to
...
`r round(summary(google_h1)$coefficients$fixed[['case_rate']], 4)` (_p_ =
`r broom.mixed::tidy(google_h1) %>% filter(term == 'case_rate') %>% pull(p.value) %>% round(3)`), 
a very small and insignificant effect. This model's phi paramter is 0.933,
which is a good indicator that adjacent time points for each county are related
and the model is specified correctly [@finch_etal14].
...
increase in the average time spent in residence of the structurally (word this
better) closest counties, an ego tends to also increase its' own stay-at-home
rate by `r broom.mixed::tidy(google_h2) %>% filter(term == 'movement_signal') %>% pull(estimate) %>% round(3)`
(_p_ < 0.001). However, this coefficient is inadequate alone
because the effect varies over time; the interaction between week and movement
signal is negative, meaning that week partially moderates the effect of the
signal. Figure \@ref(fig:plot-google-h2) illustrates this interaction. In the
early days of the Covid-19 pandemic, seeing a high rate of time spent in
residence by alters led to a high rate of staying-at-home for county egos.
However, as the pandemic progressed, these signals switched. In other words,
towards the end of 2020, other factors may have come into play and if a county
saw its' alters social distancing and spending time in residence, ego counties
spent less time at home. Theoretically, this may be because individuals saw
their adjacent communities with strong public health norms and felt safe and
justified their own deviance. However, if an ego county received signals that
...
lower rates of time spent in residence. Figure \@ref(fig:plot-google-h3)
illustrates the moderating effect that signal discordance has on signal. Under
high discordance, the social influence effect is completely moderated because
there is no clear story or norm forming. However, under low discordance, the
signal is condensed, allowing for social influence to have an effect.
Explicitly, if a county is receiving a wide array of low and high signals, their
stay at home rates won't be affected. When a signal is concentrated or in
agreement, the theoretical effect of movement signaling on the ego are the
strongest.
...
predict vaccination uptake rates. Table \@ref(tab:vacc-tab) Model 1 addresses
hypothesis 1, that relatively higher local rates of infection will lead to
increased vaccination uptake. In this first model, we see various controls
impacting vaccination rates. For instance, when controlling for everything else,
areas with higher percent White residents, areas with higher percent over 65 and
higher income areas lead to higher vaccination uptake. On the other hand, counties
with higher unemployment, higher votes for Donald J. Trump in the 2020 election,
are associated with lower vaccination rates. Interestingly, counties that search
for Fox News are more likely to have higher vaccination rates which those
searching for information about the vaccine on Google are likely to have lower
rates of vaccination. The test of Hypothesis 2 also returns surprising results:
every 1 standard deviation increase in county Covid-19 case rates is expected to
yield a `r broom.mixed::tidy(vacc_h1) %>% filter(term == 'case_rate') %>% pull(estimate) %>% round(3)`
...
variable is controlled, in this case, those related to signal. The parameter phi
is .99, which is a good indicator that adjacent time points for each county are
related and this is an appropriate modeling technique for the data [@finch_etal14].
<!-- TODO double check this number on vacc models -->
...
percent increase of vaccination rates for the ego county. The interaction of
vaccine signal can be seen in Figure \@ref(fig:plot-vacc-h2). While the
interaction is not as extreme as the results for Hypothesis 2 for Stay-at-Home
rates, there is still a distinction between high and low vaccination signals
over time. The slope for Higher Vaccination Signal is less pronounced than Low
Vaccination Signal. This is likely an artifact where some counties vaccinated
broadly early on, so their vaccination rate stays rather constant in later
weeks. However, overall, the message is clear: ego-counties who 'see' high
levels of vaccinations among their alters have higher vaccination rates; those
who don't see that social norm forming are much less likely to vaccinate.

Model 3 then models the moderation of signal direction for vaccine uptake by
diversity in signals (Discordance), Hypothesis 3.

see consistent high signals, most likely to get vaccinated, see inconsistent
signals but still high, less likely but still decently likely to raise
vaccination rates.

* won't interpret now, as there is no interaction to look at because of
convergence issues.
...
<!-- research comparing the demographics of Facebook users to actual population (JK CHECK OUT [@bailey_etal18; @jones_etal13]), it -->
...
```
```{r google-tab, dependson='google_models', cache = TRUE,  results = 'asis' }
google_labels = c(
    ...
    'week_num:movement_signal' = "Signal x Week",
...
modelsummary(list(google_h1, google_h2, google_h3),
             ...
             notes= c('Models 2-3 include a random effect for Movement Signal by FIPS',
                      'Model 1 includes a random intercept for FIPS,'), 
             estimate = "{estimate}{stars}", 
             coef_map = google_labels,
             # coef_omit = '(Cor)|(Intercept\\~google_signal\\: FIPS))|(SD \\(google_signal\\))|(SD \\(Intercept\\))|\\(SD (Observations)\\)',
             gof_omit = 'AIC|BIC|ICC|RMSE')

```
```{r vacc-tab, dependson='vacc_models', cache = TRUE,  results = 'asis' }
vacc_labels = c(
    'p_white'="Percent White",
    'p_college' = "Percent College Graduates",
    'perc_65_over' = "Percent over 65",
...
modelsummary(list(vacc_h1, vacc_h2, vacc_h3),
             ...
             # coef_omit = '(Cor)|(Intercept\\~vacc_signal\\: FIPS))|(SD \\(vacc_signal\\))|(SD \\(Intercept\\))|\\(SD (Observations)\\)',
             gof_omit = 'AIC|BIC|ICC|RMSE')
...
\newpage
# Figures {-}
```{r dag, fig.cap='Elaboratory Theoretical Model of Health Behavior Norms'}
...
```{r plot-google, cache = TRUE, fig.cap = "Stay at Home Rates over Time", dependson='plot_options'}
...
```{r plot-vacc, cache = TRUE, fig.cap = "Vaccination Rates over Time", dependson='plot_options'}
...
```{r assortativity-vacc, dependson=c('plot_options','load_vacc','discordance_extra_data_load'), cache = TRUE, fig.cap = 'How Signal Mean and Signal Assortativity are Measured', out.width="90%", fig.asp = 1.5}
...
```{r plot-google-h2, dependson=c('plot_options','google_models'), cache = TRUE, fig.cap='Predicted Values of Stay at Home Rate by Movement Signal'}
...
```{r plot-google-h3, dependson=c('plot_options','google_models'), cache = TRUE, fig.cap='Predicted Values of Stay at Home Rate Moderated'}
...
```{r plot-vacc-h2, dependson=c('plot_options','vacc_models'), cache = TRUE, fig.cap='Predicted Values of Vaccination Rate'}
...
```{r plot-vacc-h3, dependson=c('plot_options','vacc_models'), cache = TRUE, fig.cap='Predicted Values of Vaccination Rate'}
