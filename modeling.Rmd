...
There is little research on shelter-in-place or stay-at-home rates, especially during pandemics or
...
pandemic [@bargainTrustCompliancePublic2020; @bourassaStatelevelStayathomeOrders2020; @bourassaSocialDistancingHealth2020; @grossmanPoliticalPartisanshipInfluences2020; @haggerPredictingSocialDistancing2020; @hillBloodChristCompels2020; @hillNastiestQuestionDoes2021; @hillLoveThyAged2021; @huynhDoesCultureMatter2020],
...
community contexts where norms, beliefs, and values circulate [@moody19]. These
clusters of interaction are called social networks, and if “each person
continues to interact primarily with others nearby in space, the forces of
conformity will be strongest locally, leading to the emergence of clusters of
people sharing similar behavior” [@kittsAnalyticFrameworkSocial2018]. This
community interaction ultimately leads to converged communities of belief
structures with variations in divergence from the norm [@okada17;
@latane_bourgeois96; @cullum_harton07].


"Culture" diffuses through communities and social networks. Information and
opinions spread [@klar_shmargad17; @bondpolmob; @fowler2010cooperative],
behaviors are adopted [@christakis2008collective;@rosenquist2010spread;
@aral_nicolaides17; @centola10; @centola11], and there are patterns of health
contagion [@christakis_fowler07b; @cacioppo2009alone]. However, “different
things spread in different ways and to different extents” [@christakis_fowler13,
p 563] and when modeling diffusion and contagion, we must be very specific about
our scope conditions as they are relevant to our theory and not to cross
theories to infer connections where they may not exist [@kitts_quintane20].
...
@centola_macy07 theorize that simple contagions are not enough to spread
behavioral change. Simple contagions are those in which only one point of
contact is needed to receive contagion, like with infectious disease or to
learn simple bits of information. @centola_macy07's large contribution was the
...
```{r dag, fig.cap='Proposed Elaboratory Theoretical Model'}
...
```{r norm_framework, fig.cap='Preliminary model of norm formation'}
...
### Other influences on time in residence behavior
There are many other theoretical mechanisms that lead to aggregated county-level
time in residence. For instance, there is a central conflict between religious
and scientific ideologies which I theorize leads more religious counties to
reject the stay-at-home order [@evans_evans08]. Furthermore, as COVID-19 has
become a politically polarizing issue, conservative ideology and a general
mistrust of “big government” [@frank2007s] are likely to lead to resistance to
the government and scientific guidance. There may be socio-economic status
effects on time in residence because those with higher education have been shown
to be better at delaying gratification [@bembenutty2009academic], an important
concept when faced with the need to stay at home during a pandemic. With the
dramatic increase in unemployment and furloughs, counties with high rates of
poverty may abstain from staying at home because of the economic necessity to
find work, even if the work is precarious or puts them at higher risk of
infection.

# Objectives and Significance of the Study
This paper will be an important contribution to a few different fields. It will
contribute to the literature on social influence and contagion, as social
...
influence and self-selection [@lewis2012social], this project is unique because
...
### Community Integration
This project will also draw on the "Social Connectedness Index" [@Bailey2018; @facebook20] which indexes the social links between geographies by the likelihood of Facebook friends. It is an aggregated measure of Facebook friendship connections between counties It corresponds to “the (relative) probability that two arbitrary Facebook users across two geographies are friends with each other". The data is available at various geographic aggregation levels, such as U.S. zip codes or entire countries. This measure is based on user-provided information and real-time location-service data gathered by Facebook. The index also includes a measure of the "share of friends living within 100 miles (%)” aggregated to the county level. The data has been tested to show how initial COVID-19 hotspots are related to subsequent virus spread to non-hotspots, even after controlling for population density and geographic distance [@Kuchler2020]. I therefore find this to be a valid measure of both geographic and social integration.  
```{r sci, fig.cap='Relative Probability of Link to SF County, CA (Bailey et al. 2018)'}
...
### movement signal
Secondly, I plan to utilize the likelihood of a friendship connection between
counties to create a county-level social network weighted by the probability of
a tie. Using this network and the other variables above, I will model following
a similar strategy found in @aral_nicolaides17 and other articles to estimate
how the level of mobility (or lack thereof) of other counties is contagious to
the ego-county.
...
counties. I will then develop an Discordance coefficient for each county to
indicate the extent to which a given ego-network is receive diverse contagion
signals akin to @pachucki_leal20. While assortative mixing is generally a
measure used in research on homophily, in this case a low Discordance
coefficient is indicative of diverse signals, whereas high Discordance
coefficient would indicate reinforced signaling which I theorize to be akin to
complex contagion [@centola_macy07;@bojanowski_corten14].

While Discordance is important to measure the reinforcement of movement
signaling, the direction of these signals also matters. In other words, the
signals the ego-county receives from other counties may be reinforced (high
Discordance coefficient) to stay at home (positive dependent variable) or may
be reinforced (high Discordance coefficient) to remain as normal (near zero or
negative dependent variable).  _Signal-direction_ will be calculated taking an
average change in time in residences of each for each member of the ego network
with .70 or above likelihood of friendship connections to the ego.
...
```{r assortativity-vacc, dependson=c('load_vacc','discordance_extra_data_load'), cache = TRUE, fig.cap = 'How Signal Mean and Signal Assortativity are Measured'}
...
plot1d <- plot1d_data %>% 
  ...
  labs(x = 'Vaccination Signals', y = '',
       ...
       subtitle = 'Colored Point = Weighted average; Grey line = Weighted SD, Discordance',
...
I will also include religion data from the 2010 U.S. Religion Census
[@grammich_etal18] and the 2014 Religious Landscape Study
[@pewresearchcenter15]. I will measure religiosity as a mean index of six
variables ($\alpha = 0.98$), including (1) percent evangelical, (2) percent who
report attending worship services at least weekly, (3) percent who identify as
highly religious, (4) percent who say religion is very important in their lives,
(5) percent who say they pray daily, and (6) the percent who say they believe in
God with absolute certainty. This index assesses how religious a state is by
assessing self-reported religious identities, beliefs, and practices. Percent
evangelical was collected through the 2010 U.S. Religion Census: Religious
Congregations and Membership Study. The remaining variables were collected
through the 2014 Religious Landscape Study. Unfortunately, there is no
county-level religion information that I could find. I will therefore need to
use the aggregated state-level information.
...
There are two modeling strategies I plan to employ in my analysis. The first modeling strategy focuses on hypothesis 1 and uses hierarchical linear models to predict increased sheltering in place with the share of friends living within 100 miles measure as well as other control variables. I believe that using a random intercept model here is the best choice to allow both time-varying and county-level variables to predict the outcome. It must be remembered that the dependent variable comes  scaled to a baseline by Google, but I believe it is a better fit than the fixed-effects model. For the analysis, I will estimate the equation ... 
$$
\begin{aligned}
inresidence_{it} = friendshare_{i}\beta_{1}+coronacase_{it}\beta_{2}+x_{it}\beta_{3}...+u_{0i}+\epsilon_{it}
\end{aligned}
$$

where $u_{0i}$represents the total residual decomposed into county-specific time-invariant unit effects and $\epsilon_{it}$ represents the within-county idiosyncratic effects. Currently, I am treating the dependent variable as if it is continuous because it can be both positive or negative to represent changes in time spent in residence. However, it is possible the actual distribution of the data more closely resembles a Poisson distribution and requires different modeling strategy. 


$$
\begin{aligned}
inresidence_{it} = Discordance_{it}\beta_{1}+signaldirection_{it}\beta_{2}+ \\
... coronacases_{it}\beta_{3}+Discordance\times signal_{it}\beta_{4}+x_{it}\beta_{5}...+u_{0i}+\epsilon_{it}
\end{aligned}
$$
...
population in different areas [@salganik17a Section 2.3.6]. The friendship
...
[@courtois_etal12]. To my knowledge, there has been little research testing
...
shared news stories and informal knowledge networks [@kitts_quintane20].
...
data set so that it is representative [@wang_etal15; @salganik17a, Section
2.3.6]. It is therefore possible that the sampling bias may confound my results
if the age distribution through a population is associated with mobility during
COVID-19. Furthermore, Google has defined time spent in residential areas
