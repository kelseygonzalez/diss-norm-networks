...
knitr::opts_chunk$set(echo = FALSE, 
                      message = FALSE,
                      warning = FALSE)
pacman::p_load(tidyverse, here, lme4, stargazer)
...
dplyr::filter(date == lubridate::ymd("2020-04-06")) %>%
  ...
  mutate(name = ifelse(FIPS %in% sample, name, ""),
         size= ifelse(FIPS %in% sample, 2, 1),
         alpha= ifelse(FIPS %in% sample, 1, .5)) %>% 
  ggplot(aes(x = movement_signal, y = movement_assor, color = name, size = factor(size), alpha = alpha)) + geom_point() + 
  geom_text(aes(x = movement_signal + 0.5, y = movement_assor + 0.3, label = name), size = 3) + 
  theme_minimal() +
  labs() + 
...
goog <- vroom::vroom(here("data","covid-mobility", "2020_US_Region_Mobility_Report.csv"))  %>% 
...
goog %>%              
  ...
  dplyr::left_join(goog, by = c(c("fr_loc" = "FIPS"),
                   c("date_lag" = "date"))) %>%
    left_join(count_to_state, by = "FIPS") %>% 
    left_join(select(google, FIPS, date, movement_signal, movement_assor), by = c('date', 'FIPS')) %>% 
    ggplot() + 
    ggridges::geom_density_ridges(aes(x = SAH, y = name, fill = name), alpha = 0.5, show.legend = FALSE) +
        geom_segment(aes(x = (movement_signal - movement_assor) , 
                     xend = (movement_signal + movement_assor),
                     y = name, 
                     yend = name), lineend = 'butt', size = 2) +
    geom_point(aes(x = movement_signal, y = name, color = name), size = 3) +
     geom_point(aes(x = movement_signal, y = name), size = 2, color = 'black') +
    theme_minimal() + 
    labs(title = "Figure X: Movement Signals ")
```
...
```{r}
pred <- ggpredict(google_h4, c("movement_signal", "movement_assor"))
plot(pred)+ theme_minimal()
```
...
```{r}
pred <- ggpredict(vacc_h3, c("week_num", "movement_signal"))
plot(pred)+ theme_minimal()
```


```{r}
pred <- ggpredict(vacc_h4, c("movement_signal", "movement_assor"))
plot(pred)+ theme_minimal()
```