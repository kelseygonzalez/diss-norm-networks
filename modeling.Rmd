...
Descriptive statistics for the stay-at-home rate model can be found in Table \@ref(tab:google-desc-table). Descriptive statistics for the vaccination uptake model can be found in Table \@ref(tab:vacc-desc-table). 


For each case study, I employ a series of four linear mixed effects regression
models using the nlme package in R [@pinheiro_etal21; @pinheiro_bates00] to
address hypotheses 1 through 4. Linear mixed effects models are a form of
hierarchical linear models that contain both random and fixed effects. These
models treat the dependent variables as continuous. The following strategies I
...
All variables have fixed effects, meaning that the county-level exogenous
effects are controlled for when estimating the coefficient. In addition, models
are specified with a random intercept per county. The lme models are set with a
autoregressive correlation structure (`correlation = corAR1()`) to control for
...
optimization and simulated annealing (`control = lmeControl(opt = "optim")`).
This model will address Hypothesis 1. To test hypothesis 2, I estimate model 2
which builds on model 1 by first introducing the movement of vaccination
signal. Because the signal varies by week, I estimate this model with an
interaction between week and signal. Moreoever, because signal may have
different effects across different counties, I set a random effect for both
signal. A visual representation of this interaction can be seen in figures
\@ref(fig:plot-google-h2) and \@ref(fig:plot-vacc-h2). Model 3 then builds on
model 2, introducing a fixed effect for the interaction between signal and
discordance. Model 4 tests hypothesis 4. Figures \@ref(fig:plot-google-h3) and
\@ref(fig:plot-vacc-h3) depict how signal and discordance interact.
...
google_labels = c(
    ...
    'movement_signal' = "Movement Signal",
    'week_num:movement_signal' = "Movement Signal x Week",
    'movement_assor' = "Movement Discordance",
    'movement_signal:movement_assor' = "Movement Signal x Discordance")
...
The parameter phi is .95, which is a good indicator that adjacent time points for each person are related.[@finch_etal14]

\@ref(tab:google-tab)


```{r plot-google-h2, dependson='google_models', cache = TRUE, fig.cap='Predicted Values of Stay at Home Rate by Movement Signal', out.width="80%"}
...
pred2 <- pred2 %>% 
  ...
  theme(plot.title.position = "plot")
ggsave(filename = here('figs/plot-google-h2.png'), pred2)

knitr::include_graphics(here('figs/plot-google-h2.png'))

```

2. Hypothesis 2 increased average time spent in residence (signal direction) from alters will have a positive effect on time spent in residence for the ego-county
...
See Figure \@ref(fig:plot-google-h2).

```{r plot-google-h3, dependson='google_models', cache = TRUE, fig.cap='Predicted Values of Stay at Home Rate Moderated', out.width="80%"}
...
ggsave(filename = here('figs/plot-google-h3.png'), pred3)

knitr::include_graphics(here('figs/plot-google-h3.png'))
```
3. Hypothesis 3 the effect of signal direction on time spent in residence will be moderated by diversity in signals (Discordance)
...
```{r plot-vacc-h2, dependson='vacc_models', cache = TRUE, fig.cap='Predicted Values of Vaccination Rate', out.width="80%"}
...
ggsave(filename = here('figs/plot-vacc-h2.png'), pred4)
...
```{r plot-vacc-h3, dependson='vacc_models', cache = TRUE, fig.cap='Predicted Values of Vaccination Rate', out.width="80%"}
...
ggsave(filename = here('figs/plot-vacc-h3.png'), pred5)
