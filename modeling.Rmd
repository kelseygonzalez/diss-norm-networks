...
pacman::p_load(tidyverse, here, nlme, 
               ...
               kableExtra, scales, network, sna, GGally)
...
[@bailey_etal20; @akpanAssociationWhatPeople2021], but also due to habitus: the
ingrained habits, skills and dispositions of individuals and groups
[@bourdieu77]. Habitus shapes how individuals perceive different social
interactions and norms and shape how they adopt different policies, leading to
different social outcomes and a divergence of attitudes [@scottarthur_etal21;
@williams95; @madeira_etal18].
...
after the onset of the Covid-19 pandemic [@bargainTrustCompliancePublic2020;
...
@gibbons_etal21 finds that social capital had spatially inconsistent effects on
...
[@baumgaertnerInfluencePoliticalIdeology2018;
...
[@milaniVisualVaccineDebate2020], vaccination rates are a great way to
...
become politicized [@mottaRepublicansNotDemocrats2021;
...
variations in how much they diverge from the norm [@okadaStructureCulturalRejection2017;
...
behavior. Like @axelrodDisseminationCultureModel1997, this outlines
...
(@)  _Hypothesis 2_ increased average time spent in residence (signal direction) from alters will have a positive effect on stay-at-home rates the ego-county; increased vaccine uptake by alters will have a positive effect on vaccine uptake for the ego-county
...
@goldbergSocialContagionAssociative2018 propose a
...
[@mason_etal07]. When we take into account the interdependence between different
...
through schemas of perception [@houghton20]. While this cognitive theory of
...
@goldbergSocialContagionAssociative2018], I theorize that individuals interpret
...
the stay-at-home order and vaccines [@evansReligionScienceEpistemological2008]
...
ideology and a general mistrust of “big government” [@frank2007s;
@gauchatPoliticizationSciencePublic2012] are likely to lead to resistance to the
...
data, my final county sample is _n_ = `r google_raw %>% count(FIPS) %>% nrow()`
...
signals using equation \@ref(eq:networksignal) where `x` denotes the
...
The second independent variable, _Signal Discordance_, builds on the network
...
for 7 selected counties ^[Figures in this paper were all created using ggplot2
[@wickham_etal; @wickham11], patchwork [@pedersen], and ggridges [@ggridges] ].
...
data compiled by @thenewyorktimes21. It is widely acknowledged that there are biases in
...
similar population clusters [@dma_key]. To investigate the rate of _searching_ for norms
...
For a more in-depth explanation of this procedure, see @park_etal (p. 5).
...
voting data, and ecological data [@bailPrestigeProximityPrejudice2019;
...
[@pinheiro_etal21; @pinheiro_bates00] for our two cases, Stay-at-Home rates and
...
<!-- - I might have more discussion about how the findings regarding stay-at-home -->
<!-- rates are similar to / differ from those regarding vaccination rates. Why might -->
<!-- we see the differences in estimates that we see across the two outcomes? An -->
<!-- explicit comparison of how the control variables differ across the two outcomes -->
<!-- could also be interesting to see. -->



<!-- should probably interpret why vaccination is less moderated; could be type of model? -->
...
multiple big-data sources [@facebook20; @google2020]. The aggregation errors are
...
[@aralExerciseContagionGlobal2017; @sprague_house17; @bond_etal12;
@latane_etal94], combining measures of actual behavioral outcomes with data from
...
interact with each other [@houghton20; @goldbergSocialContagionAssociative2018;
@mason_etal07]. Discordance, on the other hand, looks specifically at the
