...
Write a sentence about [@bailey_etal20]
...
```{r google_models2, dependson='load_google', cache = TRUE}
google <- google %>% 
...
google <- google %>% 
  mutate(across(SAH:fox_news, ~ scale(.x, center=TRUE, scale=TRUE)))
...
vacc <- read_csv(here("data", "vacc_main.csv"), show_col_types = FALSE)  %>% 
  ...
  drop_na() %>% 
    mutate(week_num = as.numeric((date - lubridate::ymd("2020-03-02")) / 7),
           across(vacc_rate:fox_news, ~(scale(.x, scale = TRUE, center = TRUE)))) 
...
vacc <- vacc %>% 
  select(FIPS, date, vacc_rate, p_white, p_college, perc_65_over, 
         income_med, unemployed_rate,evangelical_county, 
         trump_votes_2020, fox_news, case_rate, 
         week_num, sci_prob_self,vacc_signal,vacc_assor) %>% 
  mutate(across(vacc_rate:fox_news, ~(scale(.x, scale = TRUE, center = TRUE)))) 


vacc_h1 <- lme(fixed = vacc_rate ~ p_white + p_college + perc_65_over + 
                  income_med + unemployed_rate +
                 evangelical_county + trump_votes_2020 + fox_news+
               case_rate + week_num, 
             random = ~ 1 | FIPS, 
             data = vacc,
             correlation = corAR1(), 
