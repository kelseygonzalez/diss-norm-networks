...
```{r plot_options, include=FALSE, cache = TRUE}
## Automatically use showtext to render text
font_add_google("Open Sans", "open_sans")
showtext_auto()

thematic::thematic_on(bg = 'white',
                      fg = 'black',
                      accent = my_palette[[1]], 
                      font = "open_sans",
                      sequential = my_palette,
                      qualitative = my_palette)
# 
# diss_theme <- theme_minimal() + 
#   theme(text=element_text(family="open_sans"),
#         plot.title = element_text(family="open_sans",size=25), 
#         plot.subtitle = element_text(family="open_sans",size = 20),
#         plot.caption = element_text(family="open_sans",size = 16),
#         axis.title = element_text(family="open_sans",size = 20),
#         axis.text = element_text(family="open_sans",size = 16),
...
variation in the levels of understanding of the threats of the disease
[@bailey_etal20], but it is also due to the current norms that are already in
place. TODO: discuss norms, how it's hard to diverge from current norms, habitus 
<!-- TODO -->
...
```{r dag, fig.cap='Elaboratory Theoretical Model of Health Behavior Norms',fig.align="center", out.width='80%'}
...
```{r plot-google, cache = TRUE, fig.cap = "Stay at Home over Time",fig.align="center", out.width="80%", dependson='plot_options'}

# TODO : change font to Computer Modern
...
plot_google <- vroom::vroom(here("data","covid-mobility", "2020_US_Region_Mobility_Report.csv"), show_col_types = FALSE)  %>%
  ...
  geom_line(aes(group=FIPS), color = my_palette[2], alpha = .6) + 
...
ggsave(filename = here('figs/plot-google.png'), plot_google, width = 5, height = 3)
...
```{r plot-vacc, cache = TRUE, fig.cap = "Vaccination Rates over Time", fig.align="center",out.width="80%"}
...
ggsave(filename = here('figs/plot-vacc.png'), plot_vacc, width = 5, height = 3)
...
```{r assortativity-vacc, dependson=c('plot_options','load_vacc','discordance_extra_data_load'), cache = TRUE, fig.cap = 'How Signal Mean and Signal Assortativity are Measured',fig.align="center", out.width="90%"}
...
predict stay-at-home rates, or, time spent in residence. Models 1 addresses
...
```{r plot-google-h2, dependson='google_models', cache = TRUE, fig.cap='Predicted Values of Stay at Home Rate by Movement Signal', out.width="80%", fig.align="center"}
...
ggsave(filename = here('figs/plot-google-h2.png'), pred2, width = 5, height = 3)

knitr::include_graphics(here('figs/plot-google-h2.png'))
...
```{r plot-google-h3, dependson=c('plot_options','google_models'), cache = TRUE, fig.cap='Predicted Values of Stay at Home Rate Moderated', out.width="80%",fig.align="center"}
...
pred3 <- pred3 %>% 
  ...
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), alpha = 0.5, color = NA) + 
  geom_line() + 
  labs(subtitle='Interaction of Movement Signal by Movement Signal Discordance',
       x = "Movement Signal",
       y = "Stay at Home Rate (Change from Baseline)",
       fill = "Movement Signal\nDiscordance",
       color = "Movement Signal\nDiscordance") +
...
ggsave(filename = here('figs/plot-google-h3.png'), pred3, width = 5, height = 3)
...
```{r vacc-tab, dependson='google_models', cache = TRUE,  results = 'asis' }
vacc_labels = c(
    'p_white'="Percent White",
    'p_college' = "Percent College Graduates",
    'perc_65_over' = "Percent over 65",
...
for each person are related. References: [@finch_etal14]

\@ref(tab:vacc-tab)


```{r plot-vacc-h2, dependson=c('plot_options','vacc_models'), cache = TRUE, fig.cap='Predicted Values of Vaccination Rate', out.width="80%",fig.align="center"}
...
ggsave(filename = here('figs/plot-vacc-h2.png'), pred4, width = 5, height = 3)

knitr::include_graphics(here('figs/plot-vacc-h2.png'))
```
2. Hypothesis 2 increase vaccine uptake by alters will have a positive effect on vaccine uptake for the ego-county

see a lot vaccinated, get vaccinated. see few vaccinated, don't vaccinate.
See Figure \@ref(fig:plot-vacc-h2).

```{r plot-vacc-h3, dependson=c('plot_options','vacc_models'), cache = TRUE, fig.cap='Predicted Values of Vaccination Rate', out.width="80%",fig.align="center"}
...
ggsave(filename = here('figs/plot-vacc-h3.png'), pred5, width = 5, height = 3)

knitr::include_graphics(here('figs/plot-vacc-h3.png'))
```

3. Hypothesis 3 the effect of signal direction vaccine uptake will be moderated by diversity in signals (Discordance)
