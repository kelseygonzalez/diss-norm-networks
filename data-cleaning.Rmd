...
dma <- dma %>%
  full_join(dma_key)
# 
# fox_news <- read_csv(here("data","trends","fox_news.csv")) %>%
...
weight_sd_vacc <- function(df) radiant.data::weighted.sd(df$SAH, wt = df$scaled_sci, na.rm = TRUE)
...
rm(list = c("overall_average_vacc", "sci", "vacc"))
...
rm(list = c("sci", "time_constant", "LAUS", "vacc", "covid_rate"))
