...
rm(list = c("sci_self", "count_to_state", "ACS", "votes", "gov", "evangelical_county"))
```
...
# THIS IS BROKEN as a csv AND IT WON'T LOAD AND IDK WHY so now it's just an excel :shrug:
...
covid_rate <- vroom(here("data", "covid-rates", "us-counties-2020.csv")) %>% 
  mutate(FIPS = str_split_fixed(geoid, "-", 2)[,2]) %>% 
  select(FIPS, date, case_rate = cases_avg_per_100k) 

# covid_rate %>% 
#   filter(FIPS %in% c("53061", "06059", "17031")) %>% 
#   group_by(FIPS) %>% 
#   mutate(case_rate = tidyr::replace_na(case_rate, 0),
#          case_rate_30 = zoo::rollmean(case_rate, k = 30, fill = NA),
#          case_rate_7 = zoo::rollmean(case_rate, k = 7, fill = NA)) %>% 
#   pivot_longer(cols = c(case_rate_30, case_rate_7, case_rate)) %>%
#   ggplot(aes(x=date, y = value,color = name, group = name)) + geom_line() + geom_point() +facet_wrap(~ FIPS)
...
# dma <- read_csv(here("masks", "data", "county_dma.csv")) %>% 
#   mutate(FIPS_state = case_when(STATEFP < 10 ~ paste0("0", as.character(STATEFP)),
#                                 between(STATEFP, 10, 100) ~ as.character(STATEFP)),
#          FIPS_county = case_when(CNTYFP < 10 ~ paste0("00", as.character(CNTYFP)),
#                                  between(CNTYFP, 10, 99) ~ paste0("0", as.character(CNTYFP)),
#                                  TRUE ~ as.character(CNTYFP)),
#          FIPS = paste0(FIPS_state, FIPS_county)) %>% 
#   select(FIPS, DMA)
# dma_key <- read_csv(here("masks", "data","google_dma_key.csv"))
# 
# dma <- dma %>% 
#   full_join(dma_key)
# 
# media <- read_csv(here("media","google_media_trends_region.csv")) %>% 
#   mutate(DMA_google = str_to_upper(DMA_google),
#          DMA_google = str_replace_all(DMA_google,"-", " - "),
#          DMA_google = str_replace_all(DMA_google,",", ""),
#          DMA_google = str_replace_all(DMA_google,"\\s(\\w{2})\\b", " \\(\\1\\)")) %>% 
#   full_join(dma, by = "DMA_google") %>% 
#   relocate(DMA, DMA_google) %>% 
#   janitor::clean_names() %>% 
#   rename(FIPS = fips)
...
overall_average_SAH <- google %>% 
  group_by(date) %>% 
  summarize(SAH_mean = mean(SAH, na.rm = TRUE))
...
google <-
  ...
  mutate(data = map(data, merge_alter_movement)) %>%    # merge in alter's lagged movement
  mutate(movement_signal = map_dbl(data, weight_avg_move),      # take weighted mean
         .keep = "unused")  %>% 
...
vacc <- vacc %>% 
  group_by(FIPS) %>%
  mutate(vacc_rate_2 = zoo::na.approx(vacc_rate, na.rm = TRUE),
...
overall_average_vacc <- vacc %>% 
  group_by(date) %>% 
  summarize(vacc_mean = mean(vacc_rate, na.rm = TRUE))
...
rm(list = c("overall_average_vacc", "sci"))
...
vacc <- vacc %>% 
  left_join(LAUS, by = c("FIPS", "date_month", "date_year"))
# need trends data here 
```

## save vacc
```{r vacc_save}
write_csv(vacc, here("data", "vacc_main.csv"))
rm(list = c("overall_average_vacc", "time_constant", "LAUS", "vacc"))
